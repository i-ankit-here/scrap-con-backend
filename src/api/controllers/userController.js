...
export const updateUserLocation = async (req, res, next) => {
  try {
    ...
    console.log('Updating location:', pinCode, coordinates);
    const user = await User.findById(req.user._id);
    ...
    if (coordinates) {
      // If coordinates are provided, use geocoding service to get location details
      locationDetails = await getLocationDetails({coordinates});
    } else if (pinCode) {
      // If PIN code is provided, use it to get location details
